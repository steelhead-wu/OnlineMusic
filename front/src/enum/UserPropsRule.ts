...
export const validateWhatsUp = (rule: object, value: string, callback: Function,) => {
    if (value.length > 40) {
        callback(new Error('长度不允许超过40个字符'));
        return;
    }
    callback();
}
...
export const validateBirth = (rule: object, value: string, callback: Function,) => {
    if (new Date(value).valueOf() > new Date()) {
        callback(new Error("请选择一个合适的日期"));
        return;
    }
    callback();
}
